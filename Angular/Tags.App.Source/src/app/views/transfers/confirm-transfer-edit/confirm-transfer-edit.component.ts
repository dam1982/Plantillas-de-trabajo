...
export class ConfirmTransferEditComponent implements OnInit {
  ...
  ConfirmTransfer() 
  {
    if (this.CurrentTransfer.Details.filter(x => { return x.Confirmed == null }).length > 0)
      Swal.fire({ title: "Error", text: "No ha establecido el estado de todos los paquetes. Debe confirmar o devolver la totalidad de los paquetes.", type: 'error', showCloseButton: true });
    this.onAction = true;
    this.business.ConfirmTransfer(this.CurrentTransfer).then(x => {
      ...
      Swal.fire({ title: "Error", text: x, type: 'error', showCloseButton: true });
