...
export class TransferPtEditComponent implements OnInit {
  ...
  GetWarehouses() {    
    this.business.GetSourceWarehouses(null).then(x => {
        this.optionsSourceWarehouse = x;        
    }).catch(x => {
      Swal.fire('Error', x, 'error');
    });

    this.business.GetDestinationWarehouses(null).then(x => {      
  ...
  CalcQuatities() {
    ...
    this.CurrentTransfer.Details.sort(function(a, b) {     
      return (a.PackagesInfo.PackageNumber < b.PackagesInfo.PackageNumber) ? -1 : (a.PackagesInfo.PackageNumber > b.PackagesInfo.PackageNumber) ? 1 : 0;
    });
