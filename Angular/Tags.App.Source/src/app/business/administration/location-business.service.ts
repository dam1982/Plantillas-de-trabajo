...
export class LocationBusinessService {
  ...
  public async GetLocations(locationName: string): Promise<Location[]> {
    let serviceObj = new ServiceObject("TagsLogic", 'AdminLocation', 'GetLocations');
  ...
  public SaveLocation(Location: Location): Promise<boolean> {
    let serviceObject = new ServiceObject('TagsLogic', 'AdminLocation', 'SaveLocation');
    serviceObject.Data = { location : Location };
