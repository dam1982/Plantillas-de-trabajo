...
export class FrameBusinessService {
  ...
  public async GetFrames(): Promise<Frame[]> {
    let serviceObj = new ServiceObject("TagsLogic", 'AdminFrame', 'GetFrames');
  ...
  public SaveFrame(frame : Frame): Promise<boolean> {
    let serviceObject = new ServiceObject("TagsLogic" , "AdminFrame", "SaveFrame");
  ...
  public async GetDimensions(): Promise<KeyValuePair[]> {
    let serviceObj = new ServiceObject("TagsLogic" , "AdminFrame", "GetDimensions");
    serviceObj.Data = {};
    return this.apiGatewayService.PostAction(serviceObj)
      .then(x => {
        serviceObj = x as ServiceObject;
