...
export class FrameComponent implements OnInit {
  ...
  ngOnInit() {
    this.gridMapper.Columns = [
      { prop: 'Id', name: "Id", maxWidth: 300, minWidth: 300, cellTemplate: this.IdTemplate },
      { prop: 'Code', maxWidth: 250, name: "Número", cellTemplate: this.CodeTemplate },
      { prop: 'Active', maxWidth: 150, name: 'Activo', cellTemplate: this.switchTemplate },
      { prop: 'DimensionsName', maxWidth: 250, name: "Dimensiones", cellTemplate: this.DimensionsIdTemplate },
  ...
  GetFrames(filter : any) {
    this.business.GetFrames(filter).then(x => {
...
NewLocation(isDuplicated: boolean) {
  ...
  if (isDuplicated) {
    ...
    this.frame.Id =  this.gridMapper.SelectedRows[0].Id;
