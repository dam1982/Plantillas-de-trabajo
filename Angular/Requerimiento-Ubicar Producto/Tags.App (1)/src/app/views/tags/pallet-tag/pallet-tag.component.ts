...
import { PalletTagBusinessService } from 'src/app/business/tags/pallet-tag-business.service';
...
export class PalletTagComponent implements OnInit {
  ...
  AddExternalTag(event)
  {
    ...
    var tag = event.target.value as string;
  ...
  SaveTags()
  {
    ...
    .then(x=>{
      this.toast.AddToast("Información", "Etiqueta generado con éxito.", 10, ToastyType.success, true);
      this.Cancel();
    }).catch(x=>{
