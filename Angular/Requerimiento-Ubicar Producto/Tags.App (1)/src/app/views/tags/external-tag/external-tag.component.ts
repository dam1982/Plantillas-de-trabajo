...
export class ExternalTagComponent implements OnInit {
  ...
  async AddInternalTag(event) {

    if (event.key !== "Enter")
      return;
    

    var values = (event.target.value as string).split('!');
    try {
      if (values.length != 2)
      ...
      var validateConfirm  = await  this.business.ValidateConfirmation();
  ...
  ValidateTags()
  {
    ...
    this.business.ValidateQuantityPackageByBox(this.orderId, this.internalTags).then(x=>{
      ...
      if(x == true)
      {
        ...
        Swal.fire({
          title: "Confirmación",
          text: "La cantidad de paquetes por caja excede el máximo permitido, desea generar la etiqueta de todas maneras? ",
          type: 'warning',
          showCloseButton: true,
