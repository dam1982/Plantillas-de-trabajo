 ...
 */
  ...
  async fillOptions() {
    ...
    await this.transferService.GetStates(null).then(x => {
      this.optionsStates = x;
    }).catch(x => {
      Swal.fire('', '' + x, 'error');
    });

    await this.transferService.GetUsers(null).then(x => {
      this.optionsUsers = x;
    }).catch(x => {
      Swal.fire('', '' + x, 'error');
    });
  ...
  */
  ...
  searchSourceWareHouse() {
    let value = this.SourceWareHouseSelect.searchTerm;
    if (value !== null && value.length >= 2) {
      this.transferService.GetWarehouses(value.toLowerCase()).then(x => {
        this.optionsSourceWareHouses = x;
      }).catch(x => {
        Swal.fire('', '' + x, 'error');
      });
    } else {
      this.optionsSourceWareHouses = [];
    }
  }

  searchDestinationWareHouse() {
    let value = this.DestinationWareHouseSelect.searchTerm;
    if (value !== null && value.length >= 2) {
      this.transferService.GetWarehouses(value.toLowerCase()).then(x => {
      ...
      this.optionsDestinationWareHouses = [];
    }
  }
