...
export class DispatchComponent implements OnInit {
  ...
  constructor(private business: DispatchBusinessService, private toast: ToastService) {
  ...
  manualDispatchCheckChange() {
    ...
    if (this.manualDispatchCheck) {
      ...
      this.gridMapper.Columns = [
        ...
        { width: 60, sortable: false, canAutoResize: false, draggable: false, resizeable: false, headerCheckboxable: false, checkboxable: true }
      ];
    } else {
      this.gridMapper.Columns = [
        { prop: 'BoxNumber', maxWidth: 100, name: "Caja" },
  ...
  SaveDispatch() {
    ...
    if (this.FieldTag.split('-').length != 4) { // Validacion del formato del tag 4-5-58-4

      Swal.fire('Formato incorrecto', "La etiqueta ingresada no tiene un formato válido. El formato correcto es 1-1-1-1.", 'warning');
      return false;
    }
    ...
    this.business.SaveDispatch(this.business.CurrentEnlistment.EnlistmentNumber, this.FieldTag, this.isDispatch).then(x => {
      ...
      this.toast.AddToast("Éxito", "Se obtuvo correctamente las cajas", 10, ToastyType.success, true);
    }).catch(x => {
